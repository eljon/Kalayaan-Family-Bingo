...
(function () {
  ...
  // A single tap (not a swipe, not a pinch) on the CENTRED card opens that
  // photo full-screen at full resolution. Coexists with the coverflow's own
  // 1-finger browse (which only navigates non-centred cards) and the 2-finger
  // pinch — we detect a clean single-pointer tap that lands on the centre card.
  function setupTap(dom) {
    var active = new Map();
    var cand = null; // { id, x, y, t } for a possible tap in progress
    dom.flow.addEventListener("pointerdown", function (e) {
      ...
      cand = active.size === 1
        ...
        : null;                                  // a 2nd finger → pinch, not a tap
    });
    dom.flow.addEventListener("pointermove", function (e) {
      if (!active.has(e.pointerId)) return;
      active.set(e.pointerId, e);
      if (cand && e.pointerId === cand.id &&
          Math.hypot(e.clientX - cand.x, e.clientY - cand.y) > 8) cand = null; // moved → swipe
    });
    function end(e) {
      if (!active.has(e.pointerId)) return;
      active.delete(e.pointerId);
      if (cand && e.pointerId === cand.id) {
        var c = cand; cand = null;
        if (e.type === "pointerup" && (e.timeStamp - c.t) < 500 && !cf.animating) {
          var cards = dom.flow.querySelectorAll(".cf-card");
