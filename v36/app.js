...
(function () {
  ...
  function compressImage(file, maxDim, quality) {
    return decodeWithBitmap(file, maxDim, quality)
      ...
      .catch(function () {
        // Last resort: store the original file so the photo is never lost.
        if (file && file.size && file.size <= 20 * 1024 * 1024) return file;
  ...
  function renderBoard() {
    ...
    TASKS.forEach(function (task) {
      ...
      cam.addEventListener("click", function () {
        if (done) { openModal(task.id); }        // view / replace / remove
        else { openPicker(task.id); }             // add: straight to the picker
      });
