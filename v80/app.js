...
(function () {
  ...
  var el = {
    ...
    wardSub: document.getElementById("wardSub"),
  ...
  function buildMiniCard(p) {
    ...
    // Pinned-to-cork look: a small seeded tilt and a seeded push-pin colour, so
    // each card sits at a consistent angle with its own pin every render.
    var pr = mkRng(String(seed) + "|pin");
    card.style.setProperty("--rot", (pr() * 6 - 3).toFixed(2) + "deg");
    var PINS = [
      ["#e8564a", "#ff938a", "#a52318"], ["#3f8ddb", "#93c2f2", "#215f9e"],
      ["#e6b02a", "#f7d97a", "#a97c10"], ["#49a760", "#93d6a6", "#256e38"],
      ["#9160c9", "#c6a2ec", "#5f338f"], ["#e0803f", "#f4b587", "#a5551d"]
    ];
    var pin = PINS[Math.floor(pr() * PINS.length) % PINS.length];
    card.style.setProperty("--pin", pin[0]);
    card.style.setProperty("--pinHi", pin[1]);
    card.style.setProperty("--pinLo", pin[2]);
    ...
    var motto = document.createElement("p"); motto.className = "wm-motto";
    motto.innerHTML = '<span class="c-red">Love.</span> <span class="c-teal">Share.</span> ' +
      '<span class="c-green">Serve.</span> <span class="c-navy">Together.</span>';

    paper.appendChild(name); paper.appendChild(board); paper.appendChild(motto);
    applyWornEdge(paper, seed, 240, 320);           // the saved fray / wear

    card.appendChild(paper);
    card.addEventListener("click", function () {
      if (isYou && current) renderBoard(); else openWardDetail(p);
    });
    return card;
  }
  function renderWardGrid(players) {
    ...
    if (!players.length) { el.wardEmpty.hidden = false; if (el.wardSub) el.wardSub.textContent = "Everyone's progress"; return; }
    ...
    if (el.wardSub) {
      el.wardSub.textContent = players.length + (players.length === 1 ? " family" : " families") +
  ...
  function renderBoard() {
    ...
    closeWardDetail();
    showView("board");
    activeSeed = seedOf(current);
    el.accountName.textContent = current.name;
    if (el.cardName) el.cardName.textContent = displayName(current.name);
  ...
  function openCoverflowList(doneTasks, start, animate) {
    ...
    activeSeed = seedOf(current);   // match the board's saved card look
    doneTasks.forEach(function (t) {
