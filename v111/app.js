...
(function () {
  ...
  // A strip of translucent tape (matching the wall's .wm-tape) straddling the
  // top edge of a hero centred at (cx, edgeY). Meant to be drawn inside the
  // hero's already-rotated frame so the tape sits stuck to the paper. Length,
  // offset and angle follow the wall's placement rules, freshly randomised on
  // each render: length 6.25–11.25% of the hero width, its centre kept within
  // ±30% of its own length of the hero centre, and rotated ±35°.
  function drawTape(g, cx, edgeY, heroW) {
    var len = heroW * (0.0625 + Math.random() * 0.05) * 1.5;   // 50% larger tape on shares
    var thick = len * 0.42;
    var dx = (Math.random() * 2 - 1) * 0.3 * len;
    var rot = (Math.random() * 70 - 35) * Math.PI / 180;
    var jy = (Math.random() * 0.2 - 0.05) * thick;          // slight straddle jitter
    g.save();
    g.translate(cx + dx, edgeY + jy);
    g.rotate(rot);
    ...
    var grad = g.createLinearGradient(x, y, x + len, y + thick);   // 135° tan gradient
    grad.addColorStop(0, "rgba(233,226,203,0.62)"); grad.addColorStop(1, "rgba(206,197,168,0.46)");
  ...
  function frameCardShot(shot, seed) {
    ...
    var topPad = m + Math.round(h * 0.03);          // extra room for tilt + tape
