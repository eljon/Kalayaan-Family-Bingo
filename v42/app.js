...
(function () {
  ...
  function renderBoard() {
    ...
    TASKS.forEach(function (task) {
      ...
      if (done) {
        // Completed: smaller title up top, then the photo shown as a Polaroid
        // (its own orientation), no sticker. Tap the photo to view / replace.
        cell.appendChild(title);

        var photoBtn = document.createElement("button");
        photoBtn.type = "button";
        photoBtn.className = "cell-photo-btn";
        photoBtn.setAttribute("aria-label",
          task.title + " — photo added, tap to view or replace");
        ...
        photoBtn.addEventListener("click", function () { openCoverflow(task.id); });
        ...
        // Not done: poster sticker, title, and the "Add Photo" button.
  ...
  function buildCoverflowDom() {
    ...
    remove.addEventListener("click", function () {
      ...
      removePhotoFor(t.id).then(function () {
        ...
        openCoverflowList(remaining, Math.min(idx, remaining.length - 1));
      });
      showToast("Photo removed.");
    });
  ...
  function closeCoverflow() {
    teardownCoverflow();
    if (cf.dom) cf.dom.overlay.hidden = true;
    document.body.style.overflow = "";
  }
  ...
  function openCoverflow(taskId) {
    ...
    openCoverflowList(doneTasks, start);
  }

  function openCoverflowList(doneTasks, start) {
    ...
    doneTasks.forEach(function (t) {
      ...
      card.className = "cf-slide";
      var img = document.createElement("img");
      img.alt = t.title;
      var cap = document.createElement("span");
      cap.className = "cf-slide-cap";
      cap.textContent = t.title;
      card.appendChild(img);
      card.appendChild(cap);
      dom.flow.appendChild(card);
      idbGet(photoKey(current.id, t.id)).then(function (blob) {
        if (blob) { var u = URL.createObjectURL(blob); cf.urls.push(u); img.src = u; }
      });
    });
