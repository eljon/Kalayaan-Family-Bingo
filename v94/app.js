...
(function () {
  ...
  function buildMiniCard(p) {
    ...
    // per-family seed) — position along the top, rotation and length all fresh
    // each time, while still straddling the card's top edge.
    card.style.setProperty("--tape-x", (Math.random() * 46 + 18).toFixed(1) + "%");
    card.style.setProperty("--tape-rot", (Math.random() * 70 - 35).toFixed(1) + "deg");
    card.style.setProperty("--tape-w", (Math.random() * 12 + 15).toFixed(0) + "px");
