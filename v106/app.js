...
(function () {
  ...
  function wornEdgeCanvas(seed, cw, ch) {
  ...
  function shareWholeCard() {
    ...
    ensureHtml2Canvas().then(function (h2c) {
      ...
      // no onclone distortion — capture exactly the on-screen proportions.
      return h2c(target, {
  ...
  // CSS mask is lost) and (1) clip it to the same worn-edge silhouette used on
  // screen, then (2) lay it on the board-blue page with a soft cast shadow, so
  // the shared image reads as real frayed paper — matching the live card.
  function frameCardShot(shot, seed) {
    var w = shot.width, h = shot.height;
    // Re-clip to the frayed shape. The on-screen mask is a 660x840 canvas
    // stretched to fill the paper via mask-size:100% 100%; stretch the same
    // shape over the capture so the fray lines up exactly.
    var mask = wornEdgeCanvas(seed);
    if (mask) {
    ...
    // Compose onto the page background with breathing room + a soft shadow.
    ...
    g.shadowBlur = Math.round(w * 0.045);
  ...
  if (/[?&]debug/i.test(location.search)) {
    window.__kwfb = {
      ...
      frameCardShot: frameCardShot, wornEdgeCanvas: wornEdgeCanvas
    };
  }
