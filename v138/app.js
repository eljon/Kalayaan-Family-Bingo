...
(function () {
  ...
  function zoomToBoard(fromEl, p) {
    ...
    renderBoard(p);
    if (reduce || !frame || !mini || !mini.width) return;
    requestAnimationFrame(function () {
      ...
      // Fade the surrounding chrome (top bar, progress) in as the card grows.
      var chrome = document.querySelector(".board-topbar");
      if (chrome) { chrome.style.transition = "none"; chrome.style.opacity = "0"; }
      frame.getBoundingClientRect();            // force the start state to paint
      requestAnimationFrame(function () {
      ...
      var cleanup = function () {
        ...
        frame.style.transformOrigin = ""; frame.style.opacity = "";
