...
(function () {
  ...
  // Put the bunting + FAMILY BINGO title above a screenshot of the real card.
  function frameCardCanvas(cardCanvas) {
    var W = 1080, sidePad = 46, headerH = 300, bottomPad = 46;
    var cw = W - 2 * sidePad;
    var ch = Math.round(cardCanvas.height * (cw / cardCanvas.width));
    var H = headerH + ch + bottomPad;
    var cv = document.createElement("canvas"); cv.width = W; cv.height = H;
    var g = cv.getContext("2d");
    drawShareChrome(g, W, H);
    g.drawImage(cardCanvas, sidePad, headerH, cw, ch);
    return cv;
  }

  // Share the whole bingo card: take an actual SCREENSHOT of the rendered card
  // (html2canvas), then add the bunting + title above it. Falls back to the
  // canvas redraw only if the screenshot library can't load.
  function shareWholeCard() {
    var acct = boardAcct || current;
    if (!acct || !el.share) return;
    showToast("Building your card…");
    ensureHtml2Canvas().then(function (h2c) {
      var target = document.querySelector(".card-frame") || document.querySelector(".bingo-card");
      if (!target) throw new Error("no card on screen");
      var scale = Math.min(3, Math.max(2, 1200 / (target.offsetWidth || 360)));
      return h2c(target, {
        backgroundColor: null, scale: scale, useCORS: true, logging: false,
        // Let the card size to its content in the clone so the fit-to-screen
        // squeeze doesn't clip the activity titles in the shared image.
        onclone: function (doc) {
          var g = doc.getElementById("grid");
          if (g) { g.style.gridTemplateRows = "repeat(4, auto)"; g.style.flex = "none"; }
          var f = doc.querySelector(".card-frame"); if (f) { f.style.height = "auto"; f.style.flex = "none"; }
          Array.prototype.forEach.call(doc.querySelectorAll(".cell"), function (c) { c.style.minHeight = "168px"; });
        }
      });
    }).then(function (cardCanvas) {
      showShareCanvas(frameCardCanvas(cardCanvas));
    }).catch(function () {
