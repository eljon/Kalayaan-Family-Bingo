...
(function () {
  ...
  function applyWornEdge() {
    ...
    var pad = 20, R = 30, base = 1, amp = 7; // amp = gentle swell depth in px
    ...
    var SIDE = 46, CORN = 14;
    edge(x0 + R, y0, x1 - R, y0, 0, -1, SIDE);          // top
    arc(x1 - R, y0 + R, -Math.PI / 2, 0, CORN);          // TR
    edge(x1, y0 + R, x1, y1 - R, 1, 0, SIDE);            // right
    arc(x1 - R, y1 - R, 0, Math.PI / 2, CORN);           // BR
    edge(x1 - R, y1, x0 + R, y1, 0, 1, SIDE);            // bottom
    arc(x0 + R, y1 - R, Math.PI / 2, Math.PI, CORN);     // BL
    edge(x0, y1 - R, x0, y0 + R, -1, 0, SIDE);           // left
    arc(x0 + R, y0 + R, Math.PI, Math.PI * 1.5, CORN);   // TL

    // soften the edge (no sharp line). ctx.filter blur where supported;
    // harmless if ignored — the shape is already smooth.
    try { g.filter = "blur(0.7px)"; } catch (e) {}
    g.fillStyle = "#fff";
    g.beginPath();
