...
(function () {
  ...
  function buildMiniCard(p) {
    ...
    // A strip of tape holding the card down — seeded random position along the
    // top edge, rotation, width and vertical offset, so no two look alike.
    card.style.setProperty("--tape-x", (pr() * 46 + 18).toFixed(1) + "%");
    card.style.setProperty("--tape-rot", (pr() * 70 - 35).toFixed(1) + "deg");
    card.style.setProperty("--tape-w", (pr() * 12 + 15).toFixed(0) + "px");
    // Keep the tape straddling the card's top edge (always overlapping it).
    card.style.setProperty("--tape-top", (pr() * 4 - 5).toFixed(0) + "px");
