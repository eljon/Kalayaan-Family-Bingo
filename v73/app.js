...
(function () {
  ...
  var Cloud = window.Cloud || {
    ...
    hashCode: function () { return Promise.resolve(""); },
    ...
    if (accounts.length) {
      ...
      accounts.slice().reverse().forEach(function (a) {
        ...
        open.addEventListener("click", function () {
          ...
          if (el.familyCode) { el.familyCode.value = ""; el.familyCode.focus(); }
        });
        ...
        del.addEventListener("click", function () {
          var msg = Cloud.enabled
            ? "Remove \"" + a.name + "\" from this device? Your cloud card stays — sign back in with the code to restore it."
  ...
  function finishLogin(acct, code) {
    current = acct;
    putAccount(acct);
    rememberSession(acct, code);
    setLoginBusy(false);
    if (el.loginHint) el.loginHint.textContent = "";
    if (el.familyCode) el.familyCode.value = "";
    renderBoard();
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  function enterAccount(name, code) {
    name = (name || "").trim();
    if (!name) return;
    ...
    Cloud.ready
      ...
      .then(function (codeHash) {
        ...
        if (existing) {
          if (existing.codeHash && existing.codeHash !== codeHash) { throw { kind: "badcode" }; }
          existing.local = true;
          finishLogin(existing, code);
        } else {
          var la = {
            id: id, name: name, nameKey: Cloud.nameKey(name), codeHash: codeHash,
      ...
      .catch(function (err) {
        ...
        if (el.loginHint) {
          el.loginHint.textContent = (err && err.kind === "badcode")
            ? "That name is taken and the code doesn't match. Try again, or pick a different name."
  ...
  el.newCardForm.addEventListener("submit", function (e) {
    e.preventDefault();
    enterAccount(el.familyName.value, el.familyCode ? el.familyCode.value : "");
  });
