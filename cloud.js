 ...
 *       name, nameKey, codeHash, done:{taskId:ms}, photos:{taskId:url},
 ...
 *
 * Login is name + secret code: docId is derived from the name; the code is
 * verified against codeHash (SHA-256 of nameKey + ":" + code). This is a
 * light gate suitable for low-stakes sharing — see SETUP-FIREBASE.md.
 * ---------------------------------------------------------------------- */
(function () {
  "use strict";
  ...
  function hashCode(name, code) {
    // SHA-256(nameKey:code) → hex. crypto.subtle needs a secure context
    // (https / localhost), which GitHub Pages provides.
    try {
      var data = new TextEncoder().encode(nameKey(name) + ":" + String(code));
      ...
      // Fallback for very old / insecure contexts: a weak non-crypto hash.
      var s = nameKey(name) + ":" + String(code), h = 5381, i = s.length;
