...
(function () {
  ...
  // Length follows the wall's rule (freshly randomised each render), scaled up
  // for shares: 6.25–11.25% of the hero width × 2.25.
  ...
  function drawTape(g, cx, cy, heroW, opts) {
    opts = opts || {};
    var len = heroW * (0.0625 + Math.random() * 0.05) * 2.25;
    var thick = len * 0.42;
    var spread = (opts.spreadDeg != null ? opts.spreadDeg : 35);
    ...
    var grad = g.createLinearGradient(x, y, x + len, y + thick);   // 135° green gradient
    grad.addColorStop(0, "rgba(120,183,80,0.74)"); grad.addColorStop(1, "rgba(90,152,58,0.62)");
  ...
  function frameCardShot(shot, seed) {
    ...
    g.restore();
    // A strip of tape mounting each corner (diagonally across it).
    drawTape(g, cardX,     cardY,     w, { baseDeg: 45,  spreadDeg: 10, corner: true });   // TL
    drawTape(g, cardX + w, cardY,     w, { baseDeg: -45, spreadDeg: 10, corner: true });   // TR
    drawTape(g, cardX + w, cardY + h, w, { baseDeg: 45,  spreadDeg: 10, corner: true });   // BR
    drawTape(g, cardX,     cardY + h, w, { baseDeg: -45, spreadDeg: 10, corner: true });   // BL
    g.restore();
