...
(function () {
  ...
  function sWrap(g, text, cx, y, maxW, lh) {
  ...
  // Compose the shareable card on a canvas: bunting + title + the photo as a
  // Polaroid + activity + family name + stat + motto. Returns the canvas.
  function composeShareImage(photoImg, taskTitle, familyName, doneCount) {
    var W = 1080, H = 1350;
    ...
    // Photo as a Polaroid, gently tilted
    var pad = 96, fw = W - 2 * pad, photoH = 540, capH = 158;
    var fx = pad, fy = 300, fh = photoH + capH;
    g.save();
    g.translate(W / 2, fy + fh / 2); g.rotate(-1.7 * Math.PI / 180); g.translate(-W / 2, -(fy + fh / 2));
    g.shadowColor = "rgba(20,12,4,.34)"; g.shadowBlur = 42; g.shadowOffsetY = 20;
    g.fillStyle = "#fff"; sRoundRect(g, fx, fy, fw, fh, 14); g.fill();
    g.shadowColor = "transparent"; g.shadowBlur = 0; g.shadowOffsetY = 0;
    g.save(); sRoundRect(g, fx + 26, fy + 26, fw - 52, photoH, 5); g.clip();
    sDrawCover(g, photoImg, fx + 26, fy + 26, fw - 52, photoH); g.restore();
    g.fillStyle = "#1f3a5f"; g.font = '46px "Permanent Marker", "Patrick Hand", cursive';
    sWrap(g, taskTitle, W / 2, fy + photoH + 82, fw - 80, 52);
    g.restore();

    // Family name, stat, motto
    g.textAlign = "center";
    g.fillStyle = "#1f3a5f"; g.font = '64px "Permanent Marker", "Patrick Hand", cursive';
    g.fillText(familyName, W / 2, fy + fh + 96);
    g.fillStyle = "#e8543f"; g.font = '800 40px "Nunito", system-ui, sans-serif';
    g.fillText("✓ " + doneCount + " of " + TASKS.length + " complete", W / 2, fy + fh + 156);

    g.font = '800 46px "Baloo 2", "Nunito", sans-serif';
    ...
    words.forEach(function (w) { g.fillStyle = w[1]; g.fillText(w[0], mx, H - 74); mx += g.measureText(w[0]).width + gap; });
    g.textAlign = "center";
    return cv;
  }
  ...
  // Build the featured card for a just-completed task and open the share sheet.
  function openShareCard(taskId, blob) {
    if (!el.share) return;
    var task = TASKS.filter(function (t) { return t.id === taskId; })[0];
    if (!task) return;
    var acct = current || boardAcct || {};
    var familyName = displayName(acct.name || "Family");
    var doneCount = Object.keys(acct.done || {}).length;
    var srcUrl = URL.createObjectURL(blob);
    var ready = (document.fonts && document.fonts.ready) ? document.fonts.ready : Promise.resolve();

    Promise.all([loadImage(srcUrl), ready.catch(function () {})]).then(function (r) {
      var img = r[0];
      var canvas = composeShareImage(img, task.title, familyName, doneCount);
    ...
    }).catch(function () { URL.revokeObjectURL(srcUrl); });
  }
  ...
  if (/[?&]debug/i.test(location.search)) {
    window.__kwfb = { compose: composeShareImage, loadImage: loadImage, openShareCard: openShareCard };
  }
