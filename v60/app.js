...
(function () {
  ...
  // Mobile browsers tint their toolbars (Safari: top; Chrome iOS: bottom) from
  // <meta name="theme-color">. Darken it to the backdrop while the coverflow is
  // open so BOTH bars match the overlay, then restore the app's navy on close.
  var CF_THEME = "#120e18";     // == the coverflow backdrop tint (rgb 18,14,24)
  var APP_THEME = "#1f3a5f";    // the app's normal navy
  ...
  function finishClose() {
    ...
    restoreSources();
    setThemeColor(APP_THEME);           // restore the app's navy toolbar tint
  }
  ...
  function openCoverflowList(doneTasks, start, animate) {
    ...
    dom.overlay.hidden = false;
    setThemeColor(CF_THEME);            // darken both browser toolbars to match
