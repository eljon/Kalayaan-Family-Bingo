...
(function () {
  ...
  // A faithful MINI bingo card: the family's saved seed drives the same paper
  // fray + hand-drawn cell wobble + photo tilts as their real card.
  function buildMiniCard(p) {
    ...
    var name = document.createElement("span"); name.className = "wm-name";
    name.textContent = displayName(p.name); top.appendChild(name);
    if (isYou) { var y = document.createElement("span"); y.className = "wm-you"; y.textContent = "You"; top.appendChild(y); }
    ...
    activeSeed = prevSeed;
    applyWornEdge(paper, seed, 240, 300);           // the saved fray / wear

    var meta = document.createElement("div");
    meta.className = "wm-meta" + (doneCount === TASKS.length ? " done" : "");
    meta.textContent = doneCount + "/" + TASKS.length + (doneCount === TASKS.length ? " 🏆" : "");

    card.appendChild(top); card.appendChild(paper); card.appendChild(meta);
