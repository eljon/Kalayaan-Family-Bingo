...
(function () {
  ...
  var LS_ADMIN = "kwfb.admin.v1";
  // A single ward-admin login (client-side only, same low-stakes model as the
  // family passwords) that can delete any card or any completed task.
  ...
  // A thick, hand-drawn GREEN sharpie check — procedurally built (never a font),
  // so each completed task gets its own slightly-different tick. Seeded, so a
  // given card's checks stay put across renders. Two wobbling strokes
  // (short down-stroke into a longer up-stroke) drawn thick with round caps,
  // over a soft darker under-stroke for a marker-ink feel.
  function handDrawnCheck(seed, whiteInk) {
    var rng = (seed != null) ? mkRng(String(seed) + "|chk") : Math.random;
    function rnd(a, b) { return a + rng() * (b - a); }
    var ax = rnd(16, 24), ay = rnd(50, 60);   // start, upper-left
    var vx = rnd(38, 46), vy = rnd(74, 82);   // vertex, bottom
    var ex = rnd(76, 88), ey = rnd(18, 30);   // end, upper-right (taller leg)
    var pts = [];
    function limb(x0, y0, x1, y1, n, jit) {
      for (var i = 0; i <= n; i++) {
        var t = i / n;
        pts.push([x0 + (x1 - x0) * t + (rng() - 0.5) * jit, y0 + (y1 - y0) * t + (rng() - 0.5) * jit]);
      }
    }
    limb(ax, ay, vx, vy, 3, 3.0);
    limb(vx, vy, ex, ey, 5, 3.4);
    var d = "";
    for (var i = 0; i < pts.length; i++) d += (i === 0 ? "M" : "L") + pts[i][0].toFixed(1) + " " + pts[i][1].toFixed(1) + " ";
    var w = rnd(10, 13);
    var svg;
    if (whiteInk) {
      // Bold WHITE sharpie tick over a coloured cell — a soft dark under-stroke
      // gives it just enough separation from the accent fill.
      w += 2;
      svg = '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">' +
        '<path d="' + d + '" fill="none" stroke="rgba(0,0,0,0.22)" stroke-width="' + (w + 3) + '" stroke-linecap="round" stroke-linejoin="round"/>' +
        '<path d="' + d + '" fill="none" stroke="#ffffff" stroke-width="' + w + '" stroke-linecap="round" stroke-linejoin="round"/>' +
        '</svg>';
    } else {
      svg = '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">' +
        '<path d="' + d + '" fill="none" stroke="#276b2c" stroke-width="' + (w + 3) + '" stroke-linecap="round" stroke-linejoin="round" opacity="0.30"/>' +
        '<path d="' + d + '" fill="none" stroke="#3aa03e" stroke-width="' + w + '" stroke-linecap="round" stroke-linejoin="round"/>' +
        '<path d="' + d + '" fill="none" stroke="#8fd68a" stroke-width="' + (w * 0.32) + '" stroke-linecap="round" stroke-linejoin="round" opacity="0.5"/>' +
        '</svg>';
    }
    return 'url("data:image/svg+xml,' + encodeURIComponent(svg) + '")';
  }
  ...
  if (document.fonts && document.fonts.ready) {
    ...
    // `ready` resolves only for fonts already in flight; the Wall's marker face
    // is often requested later and swaps in afterwards, reflowing a family name
    // wider. Re-fit the Wall names every time a font batch finishes loading.
  ...
  var Cloud = window.Cloud || {
    ...
    deletePlayer: function () { return Promise.resolve(); },
    deleteTask: function () { return Promise.resolve(); }
  };
  ...
  var el = {
    ...
    joinSwitchText: document.getElementById("joinSwitchText"),
  ...
  var isAdmin = false;     // ward-admin mode: can delete any card / task
  try { isAdmin = localStorage.getItem(LS_ADMIN) === "1"; } catch (e) {}
  ...
  function populateJoin() {
    ...
    updateCloudStatus();
    if (el.signOutBtn) {
      el.signOutBtn.hidden = !current && !isAdmin;
      el.signOutBtn.textContent = isAdmin ? "Exit admin mode" : "Sign out";
    }
    if (isAdmin && el.joinTitle) el.joinTitle.textContent = "Admin mode";
  ...
  // "signup" = create a new card (needs a confirmed password); "signin" = open
  // an existing card (name + password, no confirm).
  var joinMode = "signup";
  function setJoinMode(mode) {
    joinMode = (mode === "signin") ? "signin" : "signup";
    var signin = joinMode === "signin";
    if (el.confirmField) el.confirmField.hidden = signin;
    if (el.familyPass2 && signin) el.familyPass2.value = "";
    if (el.joinTitle) el.joinTitle.textContent = signin ? "Welcome back" : "Start your card";
    if (el.loginBtn) el.loginBtn.textContent = signin ? "Sign in" : "Create card";
    if (el.joinSwitchText) el.joinSwitchText.textContent = signin ? "New here?" : "Already have an account?";
    if (el.joinToggle) el.joinToggle.textContent = signin ? "Create a card" : "Sign in";
    if (el.loginHint) el.loginHint.textContent = "";
  }
  function openJoin() {
    populateJoin();
  ...
  function buildMiniCard(p) {
    ...
    card.dataset.pid = pid;   // so the card view can zoom back to this exact card
    ...
    // The paper's frayed mask insets its VISIBLE top edge ~5% (mask pad 16/336)
    // below the card box top, so anchor the tape to that 5% line — not the box
    // top, or it floats in the cork. Seat it straddling that edge, mostly on the
    // paper (centre a hair below it), still peeking a little into the cork.
    card.style.setProperty("--tape-top", "calc(5% + " + (Math.random() * 2.5 - 3.5).toFixed(1) + "px)");
    ...
    TASKS.forEach(function (t) {
      ...
      if (p.done && p.done[t.id]) {
        // Completed: no photo — the cell is a solid accent-coloured fill (same
        // colour as its outline) stamped with a bold white sharpie check.
        mc.classList.add("done");
        mc.style.backgroundImage = handDrawnBorder(accent, accent, cellSeed(t.id));
        var chk = document.createElement("div"); chk.className = "wm-check";
        chk.setAttribute("aria-hidden", "true");
        chk.style.backgroundImage = handDrawnCheck(cellSeed(t.id) + "|" + seed, true);
        mc.appendChild(chk);
      } else {
        // Unfinished: faded + desaturated so the completed cells stand out.
        mc.classList.add("todo");
        mc.style.backgroundImage = handDrawnBorder(accent, "#efe6d0", cellSeed(t.id));
    ...
    card.addEventListener("click", function () {
      // Tapping any card zooms it up into its full card view — editable if it's
      // the card you're signed in to, otherwise read-only.
      zoomToBoard(card, p);
    });
    // Admin: a delete badge to remove this whole family card.
    if (isAdmin) {
      var del = document.createElement("button");
      del.type = "button";
      ...
      del.addEventListener("click", function (e) {
        e.stopPropagation();
        if (window.confirm("Delete " + displayName(p.name) + " permanently?\nThis removes their card, photos and progress for everyone.")) {
          deleteCard(p);
        }
      });
  ...
  function zoomToBoard(fromEl, p) {
    var reduce = prefersReduced();
    var frame = document.querySelector(".card-frame");
    var mini = fromEl && fromEl.getBoundingClientRect();
    // Pre-seat the card frame at the tapped mini card's rect and hide the board
    // chrome BEFORE rendering/showing the board, so the very first painted frame
    // is already the small start state — never a full-size flash of the board.
    var chrome = document.querySelector(".board-topbar");
    var canFlip = !reduce && frame && mini && mini.width;
    if (canFlip) {
      frame.style.visibility = "hidden";        // don't paint until we've placed it
      frame.style.transition = "none";
      frame.style.opacity = "0.6";
      if (chrome) { chrome.style.transition = "none"; chrome.style.opacity = "0"; }
    }
    renderBoard(p);
    if (!canFlip) { if (frame) frame.style.visibility = ""; return; }
    // Now the board is laid out; measure it and drop the frame onto the mini
    // rect synchronously (same JS turn), so the browser paints the shrunk state
    // first rather than the full-size board.
    var full = frame.getBoundingClientRect();
    if (!full.width || !full.height) { frame.style.visibility = ""; frame.style.opacity = ""; return; }
    var s = Math.min(mini.width / full.width, mini.height / full.height);
    var dx = (mini.left + mini.width / 2) - (full.left + full.width / 2);
    var dy = (mini.top + mini.height / 2) - (full.top + full.height / 2);
    frame.style.transformOrigin = "center center";
    frame.style.transform = "translate(" + dx.toFixed(1) + "px," + dy.toFixed(1) + "px) scale(" + s.toFixed(4) + ")";
    frame.style.visibility = "";
    frame.getBoundingClientRect();              // commit the start state
    requestAnimationFrame(function () {
      frame.style.transition = "transform 0.5s cubic-bezier(.22,.61,.36,1), opacity 0.4s ease";
      frame.style.transform = "none";
      frame.style.opacity = "1";
      if (chrome) { chrome.style.transition = "opacity 0.4s ease 0.12s"; chrome.style.opacity = "1"; }
    });
    var cleanup = function () {
      frame.style.transition = ""; frame.style.transform = "";
      frame.style.transformOrigin = ""; frame.style.opacity = ""; frame.style.visibility = "";
      if (chrome) { chrome.style.transition = ""; chrome.style.opacity = ""; }
      frame.removeEventListener("transitionend", onEnd);
    };
    var onEnd = function (e) { if (e.propertyName === "transform") cleanup(); };
    frame.addEventListener("transitionend", onEnd);
    setTimeout(cleanup, 800);                   // safety net
  }

  // Leave the card view by zooming the card back down into its slot on the
  // Wall. A lightweight clone of the card is floated over the freshly-rendered
  // Wall and animated from the card's full rect down to its mini card's rect,
  // then removed — revealing the real mini card underneath.
  function zoomBackToWall() {
    var frame = document.querySelector(".card-frame");
    var acct = boardAcct;
    var fullRect = frame && frame.getBoundingClientRect();
    if (prefersReduced() || !frame || !fullRect || !fullRect.width || !acct) { renderWard(); return; }

    var ghost = frame.cloneNode(true);
    ghost.setAttribute("aria-hidden", "true");
    ghost.style.position = "fixed";
    ghost.style.left = fullRect.left + "px";
    ghost.style.top = fullRect.top + "px";
    ghost.style.width = fullRect.width + "px";
    ghost.style.height = fullRect.height + "px";
    ...
    renderWard();   // shows the Wall and (async) builds the mini cards

    var t0 = Date.now();
    (function waitForTarget() {
      var target = el.wardGrid && el.wardGrid.querySelector('.ward-mini[data-pid="' + (window.CSS && CSS.escape ? CSS.escape(acct.id) : acct.id) + '"]');
      if (!target) {
        if (Date.now() - t0 < 1600) { requestAnimationFrame(waitForTarget); return; }
        ghost.parentNode && ghost.parentNode.removeChild(ghost);   // gave up: no target
        return;
      }
      var mini = target.getBoundingClientRect();
      var s = Math.min(mini.width / fullRect.width, mini.height / fullRect.height);
      var dx = (mini.left + mini.width / 2) - (fullRect.left + fullRect.width / 2);
      var dy = (mini.top + mini.height / 2) - (fullRect.top + fullRect.height / 2);
      requestAnimationFrame(function () {
        // Stay opaque while it shrinks into the slot, then fade out at the very
        // end so the (identical) real mini card underneath takes over cleanly.
        ghost.style.transition = "transform 0.45s cubic-bezier(.4,0,.2,1), opacity 0.16s ease 0.32s";
        ghost.style.transform = "translate(" + dx.toFixed(1) + "px," + dy.toFixed(1) + "px) scale(" + s.toFixed(4) + ")";
  ...
  function renderBoard(acct) {
    ...
    TASKS.forEach(function (task) {
      ...
      if (done) {
        var cell = buildDoneCell(task, objectUrls,
          function () { openCoverflow(task.id); }, boardAcct);
        if (isAdmin) addTaskDeleteBtn(cell, task, boardAcct);
  ...
  // Admin: a small delete badge on a completed cell that removes that task.
  function addTaskDeleteBtn(cell, task, acct) {
    var del = document.createElement("button");
    del.type = "button";
    ...
    del.addEventListener("click", function (e) {
      e.stopPropagation();
      if (window.confirm('Remove "' + task.title + '" from ' + displayName(acct.name) + "?\nTheir photo and completion for this task will be deleted.")) {
        deleteTask(acct, task.id);
      }
    });
    ...
    ensureHtml2Canvas().then(function (h2c) {
      ...
      return h2c(target, {
        ...
        onclone: function (doc) {
          // Shared image: rebuild each completed cell's photo as a clean framed
          // Polaroid — a plain <div><img></div> with the white frame drawn as an
          // inline border. On screen the photo is a full-bleed cell background
          // built from a <button> + CSS mask + absolutely-filled <img> + an
          // outline overlay; html2canvas renders that stack with a grey gradient
          // block behind the photo, so for the capture we swap in this minimal,
          // artifact-free structure. Sizing the photo with width/height:auto +
          // max-* keeps its aspect ratio (no object-fit, which html2canvas
          // ignores), and no box-shadow (which html2canvas paints as a grey
          // block).
          var clonedCard = doc.querySelector(".bingo-card");
          if (clonedCard) {
            Array.prototype.forEach.call(
              clonedCard.querySelectorAll(".cell.done"),
              function (cell) {
                var btn = cell.querySelector(".cell-photo-btn");
                var img = cell.querySelector(".cell-photo");
  ...
  /* ---- Ward admin: sign in, and delete cards / tasks ---- */
  function enterAdmin() {
    isAdmin = true;
    try { localStorage.setItem(LS_ADMIN, "1"); } catch (e) {}
    current = null; forgetSession();
    setLoginBusy(false);
    if (el.loginHint) el.loginHint.textContent = "";
    if (el.familyPass) el.familyPass.value = "";
    if (el.familyPass2) el.familyPass2.value = "";
  ...
  // Delete a whole family card: its cloud doc, its photos, and the local cache.
  function deleteCard(p) {
    var id = p.id || Cloud.docId(p.name);
    var doneIds = Object.keys(p.done || {});
    showToast("Deleting " + displayName(p.name) + "…");
    var work = Promise.resolve();
    if (Cloud.enabled) {
      doneIds.forEach(function (tid) {
        work = work.then(function () { return Cloud.deletePhoto(p.name, tid); });
      });
      work = work.then(function () { return Cloud.deletePlayer(p.name); });
    }
    forgetLocal(id);   // local account cache + this device's cached photos
    work.catch(function () {}).then(function () {
      showToast(displayName(p.name) + " deleted.");
      renderWard();
    });
  }
  ...
  function enterAccount(name, pass, confirm, signInOnly) {
    name = (name || "").trim();
    if (!name) return;
    // The reserved admin login never becomes a family card.
    if (name.toLowerCase() === ADMIN_USER) {
      if (pass === ADMIN_PASS) { enterAdmin(); }
      else if (el.loginHint) el.loginHint.textContent = "That admin password doesn't match.";
      return;
    }
    setLoginBusy(true);
    Cloud.ready
      .then(function () { return resolveAccount(name, pass, confirm, !!signInOnly); })
      .then(function (acct) {
      ...
      .catch(function (err) {
        ...
        if (el.loginHint) {
          el.loginHint.textContent =
            kind === "notfound" ? "No card found with that name. Check the spelling, or create a new card."
            : kind === "badcode" ? (signInOnly ? "That password doesn't match. Try again."
                                              : "That name is taken and the password doesn't match. Try again, or pick a different name.")
            : kind === "mismatch" ? "Please type the same password in both boxes to confirm."
  ...
  if (el.joinToggle) el.joinToggle.addEventListener("click", function () {
    setJoinMode(joinMode === "signin" ? "signup" : "signin");
    try { if (el.familyName && !el.familyName.value) el.familyName.focus(); } catch (e) {}
  });
  ...
  if (el.wardJoin) el.wardJoin.addEventListener("click", function () {
    if (isAdmin) { exitAdmin(); return; }   // in admin mode this button exits it
    openJoin();
  });
  if (el.boardBack) el.boardBack.addEventListener("click", zoomBackToWall);
  if (el.boardShare) el.boardShare.addEventListener("click", shareWholeCard);

  // Pinch IN on the card view zooms it back down to its slot on the Wall.
  (function setupBoardPinch() {
    if (!el.board) return;
    var active = false, fired = false, startDist = 0;
    function dist(t) { return Math.hypot(t[0].clientX - t[1].clientX, t[0].clientY - t[1].clientY); }
    function cfOpen() { return !!(cf && cf.dom && cf.dom.overlay && !cf.dom.overlay.hidden); }
    el.board.addEventListener("touchstart", function (e) {
      if (e.touches.length === 2 && !el.board.hidden && !cfOpen()) {
        active = true; fired = false; startDist = dist(e.touches);
      }
    }, { passive: true });
    el.board.addEventListener("touchmove", function (e) {
      if (!active || fired || e.touches.length !== 2 || cfOpen()) return;
      if (startDist > 0 && dist(e.touches) / startDist < 0.7) {   // pinched in ~30%
        fired = true; active = false; zoomBackToWall();
      }
    }, { passive: true });
    var reset = function () { active = false; };
    el.board.addEventListener("touchend", reset);
    el.board.addEventListener("touchcancel", reset);
  })();
  ...
  if (el.signOutBtn) el.signOutBtn.addEventListener("click", function () {
    ...
    forgetSession(); current = null; renderWard();
  });
