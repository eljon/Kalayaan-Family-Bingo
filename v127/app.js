...
(function () {
  ...
  var el = {
    ...
    youNote: document.getElementById("youNote"),
  ...
  function renderWardGrid(players) {
    ...
    if (!players.length) {
      ...
      if (el.youNote) el.youNote.hidden = true;
      return;
    }
    ...
    // Show the "Your card!" note+arrow only when your own card is the first one.
    if (el.youNote) el.youNote.hidden = !(myId && idOf(players[0]) === myId);
