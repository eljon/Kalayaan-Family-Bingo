...
(function () {
  ...
  // ---- admin deletions ----------------------------------------------------
  // Delete a whole player card (the doc). Any photo sub-docs are removed by the
  // caller first (the client can't list a subcollection to bulk-delete it).
  Cloud.deletePlayer = function (name) {
    if (!Cloud.enabled) return Promise.resolve();
    return db.collection("players").doc(docId(name)).delete().catch(function () {});
  };
  // Remove a single completed task from a player's `done` map. A plain
  // set({merge:true}) would deep-merge and KEEP the key, so delete the field.
  Cloud.deleteTask = function (name, taskId) {
    if (!Cloud.enabled) return Promise.resolve();
    var upd = {}; upd["done." + taskId] = firebase.firestore.FieldValue.delete();
    return db.collection("players").doc(docId(name)).update(upd).catch(function () {});
  };
})();
