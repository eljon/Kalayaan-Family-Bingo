...
(function () {
  ...
  function buildMiniCard(p) {
    ...
    // Plastic push-pin colour, seeded from the festival-banner palette.
    var PINS = [
      ["#e8543f", "#a8321f"], ["#f2a03d", "#b06f18"], ["#6fae4a", "#497a2c"],
      ["#2fa3a0", "#1c6a68"], ["#1f3a5f", "#12243b"]
    ];
    var pin = PINS[Math.floor(pr() * PINS.length) % PINS.length];
    card.style.setProperty("--pin", pin[0]);
    card.style.setProperty("--pinDk", pin[1]);
    ...
    // Flat-topped plastic push pin (top disc + neck + base flange, banner colour)
    var pinEl = document.createElement("div"); pinEl.className = "wm-pin";
