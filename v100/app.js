...
(function () {
  ...
  // Featured single-photo card. The canvas ORIENTATION follows the photo
  // (portrait photo -> portrait card) and the photo is CONTAINED, never cropped.
  function composeFeaturedImage(photoImg, taskTitle, familyName, doneCount, ts) {
    var W = 1080, pad = 96, framePad = 26;
    var innerW = W - 2 * pad - 2 * framePad;
    var a = (photoImg.width && photoImg.height) ? photoImg.width / photoImg.height : 1;
    var photoW = innerW, photoH = Math.round(innerW / a);
    var maxPhotoH = 1220;                                // cap extreme portraits
    if (photoH > maxPhotoH) { photoH = maxPhotoH; photoW = Math.round(maxPhotoH * a); }
    var capH = 150, fw = W - 2 * pad, fh = 2 * framePad + photoH + capH;
    var headerH = 300, footerH = 340, fy = headerH + 8;
    var H = fy + fh + footerH;
    ...
    g.save();
    g.translate(W / 2, fy + fh / 2); g.rotate(-1.5 * Math.PI / 180); g.translate(-W / 2, -(fy + fh / 2));
    g.shadowColor = "rgba(20,12,4,.34)"; g.shadowBlur = 42; g.shadowOffsetY = 20;
    g.fillStyle = "#fff"; sRoundRect(g, pad, fy, fw, fh, 14); g.fill();
    g.shadowColor = "transparent"; g.shadowBlur = 0; g.shadowOffsetY = 0;
    var px = Math.round(W / 2 - photoW / 2), py = fy + framePad;   // centred (white letterbox if narrower)
    g.save(); sRoundRect(g, px, py, photoW, photoH, 5); g.clip();
    g.drawImage(photoImg, px, py, photoW, photoH); g.restore();
    g.fillStyle = "#1f3a5f"; g.font = '46px "Permanent Marker", "Patrick Hand", cursive';
    sWrap(g, taskTitle, W / 2, py + photoH + 68, fw - 80, 50);
    g.restore();

    g.textAlign = "center";
    g.fillStyle = "#1f3a5f"; g.font = '62px "Permanent Marker", "Patrick Hand", cursive';
    g.fillText(familyName, W / 2, fy + fh + 90);
    g.fillStyle = "#e8543f"; g.font = '800 38px "Nunito", system-ui, sans-serif';
    g.fillText("✓ " + doneCount + " of " + TASKS.length + " complete", W / 2, fy + fh + 144);
    g.fillStyle = "#8a7a5c"; g.font = '32px "Permanent Marker", "Patrick Hand", cursive';
    g.fillText(formatTs(ts), W / 2, fy + fh + 190);
    drawMotto(g, W, H - 66);
    return cv;
  }

  // The WHOLE card as a shareable image: the 3x4 grid with each family's
  // photos / stickers. Async (loads photos + stickers). Returns a Promise.
  function composeCardImage(acct) {
    var W = 1080, tmpUrls = [];
    var loaders = TASKS.map(function (t) {
      var done = !!(acct.done && acct.done[t.id]);
      if (done) {
        return getPhotoSrc(acct, t.id, tmpUrls)
          .then(function (src) { return src ? loadImage(src) : null; })
          .then(function (im) { return { t: t, done: true, img: im }; }, function () { return { t: t, done: true, img: null }; });
      }
      return loadImage(ICON_BASE + t.id + ".png")
        .then(function (im) { return { t: t, done: false, img: im }; }, function () { return { t: t, done: false, img: null }; });
    });
    ...
    return Promise.all([Promise.all(loaders), ready]).then(function (res) {
      var items = res[0];
      var margin = 60, gap = 20, cols = 3, rows = 4;
      var cellW = (W - 2 * margin - (cols - 1) * gap) / cols;
      var cellH = cellW * 1.16;
      var headerH = 300, nameBandH = 150, footerH = 120;
      var gridTop = headerH + nameBandH;
      var H = Math.round(gridTop + rows * cellH + (rows - 1) * gap + footerH);
      ...
      var doneCount = Object.keys(acct.done || {}).length;
      g.textAlign = "center";
      g.fillStyle = "#1f3a5f"; g.font = '66px "Permanent Marker", "Patrick Hand", cursive';
      g.fillText(familyName, W / 2, headerH + 62);
      g.fillStyle = "#e8543f"; g.font = '800 40px "Nunito", system-ui, sans-serif';
      g.fillText("✓ " + doneCount + " of " + TASKS.length + " complete", W / 2, headerH + 116);

      items.forEach(function (it, idx) {
        var r = Math.floor(idx / cols), c = idx % cols;
        var x = margin + c * (cellW + gap), y = gridTop + r * (cellH + gap);
        var accent = ACCENTS[it.t.accent] || "#1f3a5f";
        g.fillStyle = "#fbf3e0"; sRoundRect(g, x, y, cellW, cellH, 16); g.fill();
        g.lineWidth = 6; g.strokeStyle = accent; sRoundRect(g, x + 3.5, y + 3.5, cellW - 7, cellH - 7, 13); g.stroke();

        var ipad = 16, area = { x: x + ipad, y: y + ipad, w: cellW - 2 * ipad, h: cellH * 0.6 };
        if (it.img) {
          var ia = it.img.width / it.img.height, ra = area.w / area.h, dw, dh;
          if (it.done) {                                // photo: contain inside a white frame
            if (ia > ra) { dw = area.w; dh = dw / ia; } else { dh = area.h; dw = dh * ia; }
            var dx = area.x + (area.w - dw) / 2, dy = area.y + (area.h - dh) / 2;
            g.fillStyle = "#fff"; sRoundRect(g, dx - 5, dy - 5, dw + 10, dh + 10, 4); g.fill();
            g.drawImage(it.img, dx, dy, dw, dh);
          } else {                                      // sticker: contain, a bit smaller
            var sw = area.w * 0.66, sh = sw / ia;
            if (sh > area.h * 0.92) { sh = area.h * 0.92; sw = sh * ia; }
            g.drawImage(it.img, area.x + (area.w - sw) / 2, area.y + (area.h - sh) / 2, sw, sh);
          }
        }
        g.fillStyle = "#1f3a5f"; g.textAlign = "center";
        g.font = '700 21px "Nunito", system-ui, sans-serif';
        sWrap(g, it.t.title.toUpperCase(), x + cellW / 2, y + cellH * 0.6 + 32, cellW - 22, 24, 3);
      });
  ...
  // Share the whole bingo card (the family currently shown on the board).
  function shareWholeCard() {
    var acct = boardAcct || current;
    if (!acct || !el.share) return;
    if (el.shareBtn) { /* overlay may not be open yet */ }
    showToast("Building your card…");
    composeCardImage(acct).then(function (cv) { showShareCanvas(cv); }).catch(function () {
  ...
  function openCoverflowList(doneTasks, start, animate) {
    ...
    cf.tasks = doneTasks;
    cf.current = start;
    // Replace/Remove edit the signed-in card, so only offer them when editable.
