    ...
    ensureHtml2Canvas().then(function (h2c) {
      ...
      return h2c(target, {
        ...
        onclone: function (doc) {
          // In the shared image the done cells revert to a framed Polaroid
          // (see `.share-polaroid` in the CSS) — tidier than the on-screen
          // full-bleed photo fill when the card is shrunk into a social post.
          var clonedCard = doc.querySelector(".bingo-card");
          if (clonedCard) {
            clonedCard.classList.add("share-polaroid");
            // The per-cell CSS mask is set inline (wins over the stylesheet), and
            // some html2canvas builds paint that mask shape as a grey fill behind
            // the Polaroid — strip it so only the framed photo shows.
            Array.prototype.forEach.call(
              clonedCard.querySelectorAll(".cell.done .cell-photo-btn"),
              function (btn) {
