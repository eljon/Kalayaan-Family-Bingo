...
(function () {
  ...
  function composeFeaturedImage(photoImg, taskTitle, familyName, doneCount, ts) {
    ...
    // Polaroid on the left (vertically centred in the content band).
    var frameX = M, frameY = contentTop + Math.round((availH - frameH) / 2);
    var fcx = frameX + frameW / 2, fcy = frameY + frameH / 2;
    g.save();
    g.translate(fcx, fcy); g.rotate(-1.2 * Math.PI / 180); g.translate(-fcx, -fcy);
  ...
  function frameCardShot(shot, seed) {
    ...
    var m = Math.round(h * 0.05), gap = Math.round(h * 0.04);
    ...
    // Frayed card on the left with a soft cast shadow.
    var cardX = m, cardY = m;
    g.save();
