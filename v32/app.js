...
(function () {
  ...
  // Chunky, filled camera — the lens is knocked out (evenodd) so the button
  // colour shows through, matching the app's solid, playful button style.
  var CAM_ICON = '<svg class="cam-ic" viewBox="0 0 24 24" fill="currentColor" ' +
    'fill-rule="evenodd" clip-rule="evenodd" aria-hidden="true">' +
    '<path d="M8.4 3.6a1.4 1.4 0 0 0-1.16.62l-.9 1.35H4.2A2.6 2.6 0 0 0 1.6 8.17' +
    'v9.03a2.6 2.6 0 0 0 2.6 2.6h15.6a2.6 2.6 0 0 0 2.6-2.6V8.17a2.6 2.6 0 0 0-2.6-2.6' +
    'h-2.14l-.9-1.35a1.4 1.4 0 0 0-1.16-.62zM12 16.7a3.9 3.9 0 1 1 0-7.8 3.9 3.9 0 0 1 0 7.8z"/>' +
    '<circle cx="19.2" cy="8.6" r="1.05"/></svg>';
  var CHECK_ICON = '<svg class="cam-ic" viewBox="0 0 24 24" fill="none" ' +
    'stroke="currentColor" stroke-width="3.4" stroke-linecap="round" ' +
    'stroke-linejoin="round" aria-hidden="true"><path d="M4.5 12.5l4.5 4.5L19.5 6.5"/></svg>';
  ...
  function renderBoard() {
    ...
    TASKS.forEach(function (task) {
      ...
      // Icon area: the poster illustration for this activity
      ...
      // Camera button opens the photo picker (via the activity popup).
      var cam = document.createElement("button");
      cam.className = "cell-cam" + (done ? " is-done" : "");
      cam.type = "button";
      cam.innerHTML = (done ? CHECK_ICON : CAM_ICON) +
        '<span class="cam-tx">' + (done ? "View Photo" : "Add Photo") + "</span>";
      cam.setAttribute("aria-label",
        task.title + (done ? " — photo added, tap to view or replace" : " — add a photo"));
      cam.addEventListener("click", function () { openModal(task.id); });
